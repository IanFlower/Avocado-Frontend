<template>
    ...
    <v-dialog v-model="editbadgeDialogBox" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <Editbadge v-if="selectedbadge" :badgeId="selectedbadge.id" @badgeUpdated="refreshbadges" @close="closeEditbadgeDialog" />
          <div v-else>
            <p>Loading badge...</p>
          </div>
        </v-card-text>
    ...
    <v-dialog v-model="deletebadgeDialogBox" max-width="400px">
      <v-card>
        ...
        <v-card-text>
          <DeleteDialog :dialog="deleteDialog" :item="currentItem" :category="category" @update:dialog="deleteDialog = $event" @delete="refreshDeletebadges()" />
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
