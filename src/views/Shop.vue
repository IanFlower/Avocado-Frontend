...
const rewardsList = computed(() =>
  rewards.value.filter((reward) => {
    ...
    return reward.name.toLowerCase().includes(search.value.toLowerCase());
  })
);

// Fetch rewards and student points
onMounted(async () => {
  try {
    ...
    if (userId) {
      ...
      selectedStudentPoints.value = res.data?.[0]?.currentPoints ?? 0;
    }

  } catch (error) {
    console.error('Error fetching rewards or student points:', error);
  }
});
    ...
    <v-row align="center" justify="center">
      ...
      <v-col cols="3" class="d-flex justify-center">
        <v-btn class="text-center accent clickable-card py-8 px-13 d-flex align-center justify-center"
          elevation="6" size="x-large" @click="goToStudentHome">
          <div class="d-flex flex-column align-center">
            <span class="font-weight-bold">Points:</span>
            <span class="text-h6">{{ selectedStudentPoints }}</span>
          </div>
        </v-btn>
      </v-col>
    </v-row>
            ...
            <v-card height="35vh" width="15vw" class="secondary" @click="selectedReward = reward, modal = true">
              ...
              <v-card-actions>
                <v-row height="7vh" class="align-center mb-1 mx-auto">
                  ...
                  <v-col cols="4">
                    <v-btn :icon="`mdi-numeric-${reward.requiredPoints}`" width="2.8vw" height="2.8vw" class="accent-button">
