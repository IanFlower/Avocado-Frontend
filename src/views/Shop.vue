...
<template>
  <v-container fluid class="mt-3" style="overflow: hidden">
    <v-row align="center" justify="center">
      ...
      <v-col align="center">
        <v-btn class="text-center accent clickable-card py-8 px-13 d-flex align-center justify-center" @click="goToStudentHome"
        ...
        <v-row>
          <v-col cols="3" v-for="reward in rewardsList" :key="reward">
            <v-card height="35vh" width="15vw" class="secondary" @click="selectedReward = reward, modal = true">
              <v-img :src="reward.image_file ? `data:image/*;base64,${reward.image_file}` : NoImageAvailable" cover crop
                height="28vh" width="15vw"></v-img>
              <v-spacer></v-spacer>
              <v-card-actions>
                <v-row height="7vh" class="align-center mb-1 mx-auto">
                  ...
                  <v-col cols="4">
                    <v-btn :icon="`mdi-numeric-${reward.requiredPoints}`" width="2.8vw" height="2.8vw"
                      class="accent-button disable-events">
